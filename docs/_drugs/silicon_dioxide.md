...
description: "Silicon Dioxide 的老藥新用潛力分析。初步證據等級 L4，包含 4 個預測適應症。查看 AI 預測與臨床證據完整報告。"
parent: 中證據等級 (L3-L4)
nav_order: 156
evidence_level: L4
indication_count: 4
...
| 預測新適應症 | Active Peptic Ulcer Disease（活動性消化性潰瘍） |
| TxGNN 預測分數 | 99.93% |
| 證據等級 | L4 |
| 泰國上市 | 已上市 |
| 許可證數 | 多張（含複方製劑） |
...
**理由：**
TxGNN 預測的消化性潰瘍適應症與 Silicon Dioxide 相關化合物的制酸及黏膜保護作用機轉相符。
文獻顯示矽酸類化合物在胃腸道保護領域有應用潛力，且最新研究顯示中空介孔矽奈米粒子
可作為胃潰瘍藥物遞送的載體。

**若要推進需要：**
- 釐清 Silicon Dioxide 與含矽酸鹽制酸劑的作用差異
- 針對胃潰瘍保護作用的臨床前研究
...
**APA 格式：**
```
ThTxGNN. (2026). Silicon Dioxide老藥新用驗證報告. https://thtxgnn.yao.care/drugs/silicon_dioxide/
```

**BibTeX 格式：**
```bibtex
@misc{thtxgnn_silicon_dioxide,
  title = {Silicon Dioxide老藥新用驗證報告},
  author = {ThTxGNN Team},
  year = {2026},
  url = {https://thtxgnn.yao.care/drugs/silicon_dioxide/}
}
```
