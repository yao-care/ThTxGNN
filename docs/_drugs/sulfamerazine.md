...
Sulfamerazine 是傳統磺胺類抗菌劑，TxGNN 預測其可能對結膜炎（conjunctivitis）有治療潛力，歷史文獻支持其曾用於砂眼治療，但所有泰國許可證均已註銷。
</p>
...
**理由：**
雖然歷史文獻支持 Sulfamerazine 在砂眼（披衣菌性結膜炎）治療中的應用，但此藥物在泰國已無有效許可證，且現已有更安全有效的抗生素選擇（如 Azithromycin）。砂眼在泰國已非主要公共衛生問題，重新開發此藥物的臨床價值有限。
...
**APA 格式：**
```
ThTxGNN. (2026). Sulfamerazine老藥新用驗證報告. https://thtxgnn.yao.care/drugs/sulfamerazine/
```

**BibTeX 格式：**
```bibtex
@misc{thtxgnn_sulfamerazine,
  title = {Sulfamerazine老藥新用驗證報告},
  author = {ThTxGNN Team},
  year = {2026},
  url = {https://thtxgnn.yao.care/drugs/sulfamerazine/}
}
```
