...
def normalize_ingredient(name: str) -> str:
    ...
    # 移除劑量資訊 (例如 "500 mg", "20 MG", "0.5 G", "100 MCG/ML")
    # 格式: 數字 + 空格 + 單位
    dosage_pattern = r"\s+\d+\.?\d*\s*(mg|g|mcg|ug|ml|l|iu|unit|units|%|mg/ml|mcg/ml|g/ml)\b"
...
def remove_dosage(name: str) -> str:
    """移除藥品名稱中的劑量資訊
    ...
    # 移除劑量資訊 (例如 "500 mg", "20 MG", "0.5 G", "100 MCG/ML")
    name = re.sub(r"\s+\d+\.?\d*\s*(mg|g|mcg|ug|ml|l|iu|unit|units|%|mg/ml|mcg/ml|g/ml)\b", "", name, flags=re.IGNORECASE)
...
def get_all_synonyms(ingredient_str: str) -> List[Tuple[str, List[str]]]:
    ...
    for part in parts:
        ...
        # 移除劑量資訊
        main_name = remove_dosage(main_name)
