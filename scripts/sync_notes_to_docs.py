...
def extract_evidence_level(content: str) -> str:
    ...
    # Look for "| ระดับหลักฐาน | L1 |" or "| Evidence Level | L1 |" pattern
    match = re.search(r'\|\s*(?:ระดับหลักฐาน|Evidence Level|證據等級)\s*\|\s*(L[1-5])\s*\|', content, re.IGNORECASE)
    if match:
...
def get_parent_by_level(level: str) -> str:
    ...
    if level in ("L1", "L2"):
        return "หลักฐานระดับสูง (L1-L2)"
    elif level == "L3":
        return "หลักฐานระดับกลาง (L3-L4)"
    elif level == "L4":
        return "หลักฐานอ่อน (L4)"
    else:
        return "การคาดการณ์จากโมเดล (L5)"
...
def sync_notes_to_docs():
    ...
    print(f"พบ {len(drug_dirs)} โฟลเดอร์ยา")
    ...
    for i, drug_dir in enumerate(drug_dirs, 1):
        ...
        if not notes_path.exists():
            errors.append(f"{drug_name}: ไม่พบบันทึก")
            continue
...
ระดับหลักฐาน: **{evidence_level}** | ข้อบ่งใช้ที่ทำนาย: **{indication_count}** รายการ
{{: .fs-6 .fw-300 }}
