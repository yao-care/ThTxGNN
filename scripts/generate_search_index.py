...
1. Drug name search (English INN + Thai brand names)
...
def extract_brand_names(bundle: dict) -> list:
    """Extract Thai brand names from ThaiFDA data."""
    brand_names = set()

    thaifda = bundle.get("thaifda", {})
    licenses = thaifda.get("licenses", [])

    for license_info in licenses:
        name = license_info.get("name", "")
        if name:
            # Extract Thai characters only (Thai Unicode range: 0E00-0E7F)
            thai_name = re.sub(r'[^\u0e00-\u0e7f]', '', name)
            if thai_name and len(thai_name) >= 2:
                brand_names.add(thai_name)
...
def load_all_bundles() -> list:
    ...
    if not BUNDLES_DIR.exists():
        print(f"ไม่พบไดเรกทอรี bundles: {BUNDLES_DIR}")
        return bundles
    ...
    for drug_dir in BUNDLES_DIR.iterdir():
        ...
        try:
            ...
            print(f"ข้อผิดพลาดในการโหลด {bundle_file}: {e}")
...
def main():
    print("=" * 60)
    print("สร้างดัชนีการค้นหา")
    print("=" * 60)

    # Load all bundles
    bundles = load_all_bundles()
    print(f"โหลด {len(bundles)} drug bundles")

    # Build search index
    index = build_search_index(bundles)
    print(f"สร้างดัชนีด้วย {index['drug_count']} ยา และ {index['indication_count']} ข้อบ่งใช้")
    ...
    for drug in index["drugs"][:3]:
        print(f"  - {drug['name']} ({drug['level']}): {len(drug['indications'])} ข้อบ่งใช้")
        if drug['brands']:
            print(f"    ชื่อการค้า: {', '.join(drug['brands'])}")

    print("\nตัวอย่างข้อบ่งใช้:")
    for ind in index["indications"][:3]:
        print(f"  - {ind['name']} ({ind['level']}): {ind['count']} ยา")
